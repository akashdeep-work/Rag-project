...
class Indexer:
    ...
    def _filter_relevant(self, candidates: List[SearchCandidate]) -> List[SearchCandidate]:
        filtered: List[SearchCandidate] = []
        for cand in candidates:
            relevance_score = cand.score if cand.score is not None else cand.vector_score
    ...
    def index_file(self, file_path: Path, chunk_size: int = 500, chunk_overlap: int = 50):
        ...
        if not self._should_index(str(file_path), source_id):
            ...
            return
        print(f"File indexing {file_path} or {file_path.resolve()}")
        self.index_sources([str(file_path.resolve())], chunk_size, chunk_overlap)
    ...
    def search(self, query: str, k: int = 10) -> List[SearchCandidate]:
        ...
        dense_results = self.db.search(q_emb, k=initial_k)
        dense_candidates = [
            SearchCandidate(chunk_id=uid, vector_score=score, metadata=meta or {})
            for uid, score, meta in dense_results
        ]

        reranked = self.reranker.rerank(query, dense_candidates, top_k=max(k, RERANK_TOPK))
        filtered = self._filter_relevant(reranked)
        return filtered[:k]
